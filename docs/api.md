...
```cpp
// M A N D A T O R Y ! ! !
void onConnect(bool session); // session = whether connection has started with a dirty session
void onMessage(const char* topic, const uint8_t* payload, size_t len,uint8_t qos,bool retain,bool dup);
// O P T I O N A L
...
*/
void onError(uint8_t error,int info); // info is additional information about the error whose code is one of:
/*
